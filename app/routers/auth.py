...
def signup_user(request:Request, db:Annotated[Session, Depends(get_session)], username: Annotated[str, Form()], email: Annotated[str, Form()], password: Annotated[str, Form()],):
    try:
...
def get_user_by_id(db: Annotated[Session, Depends(get_session)], user:AuthDep):
    return user
